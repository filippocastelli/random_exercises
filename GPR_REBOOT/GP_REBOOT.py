...
class GPR_reboot(object):
    ...
    def update_params(self, newparams_dict):
        ...
        assert len(self.kernel_arguments) == len(
            newparams_names
        ), "wrong number of parameters for kernel!"
        assert not sum(
            [not i in newparams_names for i in self.kernel_arguments]
    ...
    # > GAUSSIAN KERNEL
    @classmethod
    def gaussian_kernel(cls, data1, data2, length=1, const=1):
    ...
    # > PERIODIC KERNEL
    @classmethod
    def periodic_kernel(cls, data1, data2, period=1, length=1, const=1):

        dist = cls.difference_mat(data1, data2)
        
        k = np.square(const) * np.exp(
            -2 * np.square(np.sin((np.pi / period) * dist / np.square(length)))
        )
    ...
    @classmethod
    def rational_quadratic(cls, data1, data2, alpha=1, length=1, const=1):

        squared_diff = cls.difference_mat(data1, data2) ** 2

        k = np.square(const) * np.power(
            (1 + squared_diff / (2 * alpha * np.square(length))), -alpha
        )
...
#    > MAUNA LOA KERNEL
    ...
    def mauna_loa_example_kernel(
        ...
        gaussian_component = cls.gaussian_kernel(
            data1=data1, data2=data2, length=RBF_length, const=RBF_const
        )

        periodic_component0 = cls.gaussian_kernel(
            data1=data1, data2=data2, length=RBFperiodic_length, const=RBFperiodic_const
        )

        periodic_component1 = cls.periodic_kernel(
            data1=data1, data2=data2, period=1, length=PERIODIC_length, const=1
        )

        rational_quadratic_component = cls.rational_quadratic(
            data1=data1,
            data2=data2,
            alpha=RADQUAD_shape,
        ...
        noise_component = cls.gaussian_kernel(
            data1=data1, data2=data2, length=RBFnoise_length, const=const
        )
...
#                > get_Knoise(K,R,N)
    ...
    def optimizer(self, mode="brute", ranges_dict=None, Ns=100, output_grid=False):
